...
echo "$($PSQL "SELECT DISTINCT name FROM teams JOIN games ON teams.team_id = games.winner_id WHERE games.round = 'Eighth-Final' AND games.year = '2014' UNION SELECT DISTINCT name FROM teams JOIN games ON teams.team_id = games.opponent_id WHERE games.round = 'Eighth-Final' AND games.year = '2014'")"
